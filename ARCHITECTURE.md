...
| Release flow       | `release-policy.cjs` + `release-preflight.js` (stable override and `.p8`/dev-token leak gates), `release-warning.js`, `ensure-draft-release.cjs`, `wait-for-draft-release.cjs`, `publish-release.cjs`, `gpg-sign.js`, `updater-signature-verifier.js` + `src-tauri/examples/verify_updater_signatures.rs`, `generate-updater-manifests.js` (`latest-windows-*.json`), `validate-updater-manifest.js`, `verify-release-draft.js`, `verify-release-published.js`, `post-release-assets.js` + `finalize-release-assets.js` (`AFTER_PACK_LOC` archive mirror), `run-release.js`, `github-cli.cjs` |
